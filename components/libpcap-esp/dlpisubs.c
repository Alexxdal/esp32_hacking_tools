...
#define	PKTBUFSIZE	(MAXDLBUF * sizeof(bpf_unsigned int32))
...
#endif
//#include <sys/dlpi.h> AD COMMENTATO
//#include <sys/stream.h> AD COMMENTATO
...
#include <stdlib.h>
#include <string.h>
//#include <stropts.h> AD COMMENTATO
#include <unistd.h>
 ...
 */
int
pcap_process_pkts(pcap_t *p, pcap_handler callback, unsigned char *user,
	int count, unsigned char *bufp, int len)
{
	struct pcap_dlpi *pd = p->priv;
	...
	while (bufp < ep) {
		...
		if (pcap_filter(p->fcode.bf_insns, pk, origlen, caplen)) {
			...
			//if (pkthdr.caplen > (bpf_unsigned int32)p->snapshot) AD COMMENTATO
			//	pkthdr.caplen = (bpf_unsigned int32)p->snapshot; AD COMMENTATO
			//(*callback)(user, &pkthdr, pk); AD COMMENTATO
 ...
 */
int
pcap_process_mactype(pcap_t *p, unsigned int mactype)
{
	int retv = 0;
//AD COMMENTATO
// 	switch (mactype) {
...
{
	struct timeval to;
	bpf_unsigned int32 ss, chunksize;
